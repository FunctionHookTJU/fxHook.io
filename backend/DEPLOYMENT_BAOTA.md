# fxHook.io 日记系统 - 宝塔面板部署文档

> 适用于 Ubuntu 22.04 + 宝塔面板环境

## 🐳 推荐方案：Docker 部署（最简单）

### 快速开始

```bash
# 1. 进入项目目录
cd /www/wwwroot/fxHook.io

# 2. 使用宝塔专用配置启动（无 Nginx 容器）
docker compose -f docker-compose.baota.yml up -d --build

# 3. 检查状态
docker compose -f docker-compose.baota.yml ps
```

这将启动：
- **MongoDB** 数据库（容器内部）
- **Backend** API 服务（端口 3000）

前端静态文件由宝塔的 Nginx 直接服务。

### 宝塔 Nginx 配置

在宝塔面板中配置站点的 Nginx：

1. **网站** → 选择你的站点 → **设置** → **配置文件**
2. 添加以下反向代理配置：

```nginx
# API 反向代理到 Docker 后端
location /api/ {
    proxy_pass http://127.0.0.1:3000;
    proxy_http_version 1.1;
    proxy_set_header Upgrade $http_upgrade;
    proxy_set_header Connection 'upgrade';
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Proto $scheme;
    proxy_cache_bypass $http_upgrade;
}
```

3. 保存并重载 Nginx

### 访问测试

```bash
# 测试后端 API
curl http://localhost:3000/api/health

# 访问网站（使用你的域名或 IP）
http://你的域名
```

### 常用命令

```bash
# 查看日志
docker compose -f docker-compose.baota.yml logs -f

# 重启服务
docker compose -f docker-compose.baota.yml restart

# 停止服务
docker compose -f docker-compose.baota.yml down

# 更新代码后重新部署
git pull origin dynamic
docker compose -f docker-compose.baota.yml up -d --build
```

---

## 传统方案：手动部署

如果不使用 Docker，可以按以下步骤手动部署：

## 系统架构

- **前端**: 静态HTML页面 + JavaScript
- **后端**: Node.js + Express
- **数据库**: MongoDB
- **服务器**: Ubuntu 22.04 + 宝塔面板

---

## 一、宝塔面板准备工作

### 1. 安装宝塔面板（如未安装）

```bash
wget -O install.sh https://download.bt.cn/install/install-ubuntu_6.0.sh && sudo bash install.sh ed8484bec
```

安装完成后记录：
- 宝塔面板地址（如：`http://your-ip:8888`）
- 用户名和密码

### 2. 登录宝塔面板

访问面板地址，使用账号密码登录

---

## 二、安装必要软件

### 1. 在宝塔面板安装软件

**软件商店** → 搜索并安装以下软件：

#### ① Nginx（必装）
- 版本：1.22 或更高
- 用途：Web服务器和反向代理

#### ② PM2管理器（必装）
- 用途：Node.js进程管理
- 在 **软件商店** → **运行环境** → 搜索 "PM2管理器" → 安装

#### ③ MongoDB（必装）
- 版本：推荐 5.0 或更高
- 在 **软件商店** → **数据库** → 搜索 "MongoDB" → 安装
- 安装完成后记录用户名和密码（如果设置了）

**如果软件商店没有MongoDB**，手动安装：

```bash
# SSH连接到服务器
cd /opt

# 下载MongoDB
wget https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-ubuntu2204-7.0.5.tgz

# 解压
tar -zxvf mongodb-linux-x86_64-ubuntu2204-7.0.5.tgz
mv mongodb-linux-x86_64-ubuntu2204-7.0.5 mongodb

# 创建数据和日志目录
mkdir -p /data/mongodb/data
mkdir -p /data/mongodb/logs

# 创建配置文件
cat > /opt/mongodb/mongodb.conf << EOF
systemLog:
  destination: file
  path: /data/mongodb/logs/mongodb.log
  logAppend: true
storage:
  dbPath: /data/mongodb/data
  journal:
    enabled: true
net:
  port: 27017
  bindIp: 127.0.0.1
EOF

# 创建systemd服务
cat > /etc/systemd/system/mongodb.service << EOF
[Unit]
Description=MongoDB Database Server
After=network.target

[Service]
Type=forking
User=root
ExecStart=/opt/mongodb/bin/mongod --config /opt/mongodb/mongodb.conf --fork
ExecStop=/opt/mongodb/bin/mongod --shutdown --config /opt/mongodb/mongodb.conf
Restart=on-failure

[Install]
WantedBy=multi-user.target
EOF

# 启动MongoDB
systemctl daemon-reload
systemctl start mongodb
systemctl enable mongodb

# 检查状态
systemctl status mongodb
```

---

## 三、上传项目代码

### 方式1：使用宝塔面板文件管理器

1. 打开 **文件** 菜单
2. 导航到 `/www/wwwroot/`
3. 点击 **上传** → 上传整个 `fxHook.io` 项目压缩包
4. 解压文件

### 方式2：使用Git（推荐）

在宝塔面板 **终端** 或SSH中执行：

```bash
cd /www/wwwroot/
git clone https://github.com/FunctionHookTJU/fxHook.io.git
cd fxHook.io
git checkout dynamic
```

---

## 四、配置后端服务

### 1. 进入后端目录

```bash
cd /www/wwwroot/fxHook.io/backend
```

### 2. 配置环境变量

使用宝塔面板文件管理器编辑，或命令行：

```bash
cp .env.example .env
nano .env
```

修改 `.env` 内容：

```env
# 服务器端口
PORT=3000

# MongoDB连接（本地）
MONGODB_URI=mongodb://127.0.0.1:27017/fxhook_diary

# JWT密钥（必须修改！使用随机字符串）
JWT_SECRET=请修改为你自己的超长随机字符串至少32位

# 管理员密码（必须修改！）
ADMIN_PASSWORD=请设置你的强密码

# CORS配置（你的域名）
ALLOWED_ORIGINS=https://fxhook.fun,https://www.fxhook.fun,https://functionhooktju.github.io
```

保存退出（Ctrl+X → Y → Enter）

### 3. 安装依赖

#### 方法A：在宝塔终端安装（推荐）

打开 **终端**：

```bash
cd /www/wwwroot/fxHook.io/backend
npm install
```

#### 方法B：使用PM2管理器安装

1. 打开 **软件商店** → **PM2管理器**
2. 点击 **项目管理** → **添加项目**
3. 填写信息：
   - **项目名称**：fxhook-diary
   - **项目路径**：`/www/wwwroot/fxHook.io/backend`
   - **启动文件**：`/www/wwwroot/fxHook.io/backend/server.js`
   - **运行目录**：`/www/wwwroot/fxHook.io/backend`
4. 点击 **安装依赖**（会自动运行npm install）

### 4. 启动Node.js服务

#### 使用PM2管理器启动（推荐）

1. 在PM2管理器的项目列表中找到 `fxhook-diary`
2. 点击 **启动**
3. 设置 **开机自启**

#### 或使用命令行：

```bash
cd /www/wwwroot/fxHook.io/backend
pm2 start server.js --name fxhook-diary
pm2 save
pm2 startup
```

### 5. 检查服务状态

在宝塔终端执行：

```bash
pm2 status
pm2 logs fxhook-diary
```

或在PM2管理器中查看日志

---

## 五、配置Nginx反向代理

### 1. 添加网站

1. 打开 **网站** 菜单
2. 点击 **添加站点**
3. 填写信息：
   - **域名**：`api.fxhook.cn`（你的API域名）
   - **根目录**：随便选一个，稍后会修改
   - **PHP版本**：纯静态
4. 点击 **提交**

### 2. 配置反向代理

1. 在网站列表中找到 `api.fxhook.cn`
2. 点击 **设置**
3. 点击左侧 **反向代理**
4. 点击 **添加反向代理**
5. 填写：
   - **代理名称**：fxhook-api
   - **目标URL**：`http://127.0.0.1:3000`
   - **发送域名**：`$host`
6. **高级设置**（可选）：

```nginx
proxy_set_header X-Real-IP $remote_addr;
proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
proxy_set_header X-Forwarded-Proto $scheme;
proxy_set_header Host $host;
proxy_http_version 1.1;
proxy_set_header Upgrade $http_upgrade;
proxy_set_header Connection "upgrade";
```

7. 点击 **提交**

### 3. 配置HTTPS（免费SSL证书）

1. 在网站设置中点击 **SSL**
2. 选择 **Let's Encrypt**
3. 勾选你的域名
4. 点击 **申请**
5. 等待证书签发（通常几秒钟）
6. 开启 **强制HTTPS**

---

## 六、部署前端

### 方式1：使用现有网站（如果已经有）

如果你已经有主网站（如 `fxhook.cn`）：

1. 前端文件已经在 `/www/wwwroot/fxHook.io/` 目录
2. 在宝塔面板 **网站** → 找到你的主网站
3. 点击 **设置** → **网站目录**
4. 将运行目录设置为 `/www/wwwroot/fxHook.io`

### 方式2：创建新网站

1. 点击 **添加站点**
2. 填写：
   - **域名**：`fxhook.cn` 或 `www.fxhook.cn`
   - **根目录**：`/www/wwwroot/fxHook.io`
   - **PHP版本**：纯静态
3. 点击 **提交**
4. 配置SSL证书（同上）

### 修改API地址

使用宝塔文件管理器或命令行编辑：

```bash
nano /www/wwwroot/fxHook.io/scripts/diaryAPI.js
```

修改第2行：

```javascript
// const API_BASE_URL = 'http://localhost:3000/api'; // 开发环境
const API_BASE_URL = 'https://api.fxhook.cn/api'; // 改为你的API域名
```

保存文件

---

## 七、数据迁移（迁移现有日记）

### 1. 创建迁移脚本

在宝塔文件管理器或终端创建文件：

```bash
nano /www/wwwroot/fxHook.io/backend/migrate.js
```

粘贴以下内容，并**填入你的所有日记数据**：

```javascript
require('dotenv').config();
const mongoose = require('mongoose');
const Diary = require('./models/Diary');

// 你的所有日记数据
const existingDiaries = [
  {
    date: new Date('2026-01-21'),
    title: '',
    content: '今天终于把前几门恶心的课给考完了，大物竟然考的都会，只是概率论出成绩，坠机了。\n明天GTM8+ 11:00明日方舟终末地开服，期待中......',
    images: []
  },
  {
    date: new Date('2026-01-16'),
    title: '',
    content: '我恨计算机系统基础2。我恨数据结构。来自考试前深夜的怒吼。本来分配五天时间准备一半学csapp另一半学数据结构的，结果光复习csapp的ppt就用掉了四天的时间。数据结构还有那么多贼几把长的纸面作业要写，考完数据结构还要复习英语和大物，谁他妈有功夫写你那纸面作业啊，我能凑合着给你写写算法题就不错了，他妈的又臭又长。去死吧去死去死去死！',
    images: []
  },
  {
    date: new Date('2026-01-01'),
    title: '',
    content: '2026新年快乐！也是终于熬到copilot免费额度刷新的时候了。上个月月初用掉了太多额度来做那个没用的网页。最近也是到期末复习的阶段了，也是十多天没修理过这个网页了，买了服务器之后也没怎么在意把这个网页部署上去，在那个服务器里部署了原版mc，却因为内存太少总是闪退，于是服务器已经连续很多天处于一个闲置的状态。有时间要搞一个普通tcp协议的小程序玩玩。',
    images: []
  },
  {
    date: new Date('2025-12-20'),
    title: '',
    content: '自从把服务器的镜像换成ubuntu之后，我突然就感受到linux系统命令行操作的美感了。宝塔面板又缓解了linux系统查看文件结构不方便的难题，而且宝塔面板的命令行是用8888端口直连的，不会像ssh那样占用大量资源。我真的要爱上linux系统了。',
    images: []
  }
  // ... 继续添加你的所有日记
  // 可以从原 diary.html 复制内容
];

async function migrate() {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('✅ 数据库连接成功');
    
    // 清空现有数据（如果需要）
    // await Diary.deleteMany({});
    // console.log('已清空现有数据');
    
    // 插入数据
    const result = await Diary.insertMany(existingDiaries);
    console.log(`✅ 成功迁移 ${result.length} 条日记！`);
    
    process.exit(0);
  } catch (err) {
    console.error('❌ 迁移失败:', err);
    process.exit(1);
  }
}

migrate();
```

### 2. 运行迁移

在宝塔终端执行：

```bash
cd /www/wwwroot/fxHook.io/backend
node migrate.js
```

看到 "✅ 成功迁移 XX 条日记！" 表示成功

---

## 八、测试系统

### 1. 测试后端API

在宝塔终端或浏览器测试：

```bash
# 健康检查
curl https://api.fxhook.cn/api/health

# 获取日记列表
curl https://api.fxhook.cn/api/diaries
```

### 2. 测试前端

浏览器访问：

- **日记页面**：`https://fxhook.cn/pages/diary.html`
- **管理后台**：`https://fxhook.cn/pages/diary-admin.html`

### 3. 测试管理功能

1. 访问管理后台
2. 使用 `.env` 中设置的密码登录
3. 尝试创建、编辑、删除日记

---

## 九、宝塔面板常用操作

### 1. 查看Node.js日志

**软件商店** → **PM2管理器** → 点击项目名 → **日志**

### 2. 重启Node.js服务

**PM2管理器** → 找到项目 → 点击 **重启**

或终端：
```bash
pm2 restart fxhook-diary
```

### 3. 查看MongoDB状态

**软件商店** → **已安装** → 找到MongoDB → 点击 **设置** → **服务**

### 4. 备份数据库

#### 方法A：使用宝塔计划任务

1. **计划任务** → **添加计划任务**
2. **任务类型**：Shell脚本
3. **任务名称**：备份MongoDB
4. **执行周期**：每天凌晨3点
5. **脚本内容**：

```bash
#!/bin/bash
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR="/www/backup/mongodb"
mkdir -p $BACKUP_DIR

mongodump --db=fxhook_diary --out=$BACKUP_DIR/backup_$DATE

# 只保留最近7天的备份
find $BACKUP_DIR -type d -mtime +7 -exec rm -rf {} \;
```

#### 方法B：手动备份

```bash
# 备份
mongodump --db=fxhook_diary --out=/www/backup/mongodb/backup_$(date +%Y%m%d)

# 恢复
mongorestore --db=fxhook_diary /www/backup/mongodb/backup_20260130/fxhook_diary/
```

### 5. 防火墙设置

1. **安全** → **防火墙**
2. 确保开放的端口：
   - 80（HTTP）
   - 443（HTTPS）
   - 8888（宝塔面板）
   - 22（SSH）
3. **不要**开放3000端口（Node.js通过Nginx代理访问）
4. **不要**开放27017端口（MongoDB仅本地访问）

---

## 十、安全加固

### 1. 修改宝塔面板端口

**面板设置** → **面板端口** → 改为自定义端口（如 `18888`）

### 2. 开启宝塔面板BasicAuth

**面板设置** → **安全设置** → 开启 **BasicAuth认证**

### 3. 设置IP白名单（可选）

**面板设置** → **授权IP** → 添加你的常用IP

### 4. 修改SSH端口

**安全** → **SSH** → 修改SSH端口（建议非默认22端口）

### 5. 开启IP限流（防DDoS）

**安全** → **系统加固** → 开启相关保护

---

## 十一、常见问题

### Q1: PM2管理器找不到/安装失败

**解决方案**：
```bash
# 手动安装PM2
npm install -g pm2

# 然后在宝塔面板重新扫描
```

### Q2: MongoDB启动失败

**解决方案**：
```bash
# 查看日志
tail -f /data/mongodb/logs/mongodb.log

# 检查端口占用
netstat -tuln | grep 27017

# 重启MongoDB
systemctl restart mongodb
```

### Q3: 反向代理504错误

**解决方案**：
1. 检查Node.js服务是否运行：`pm2 status`
2. 查看Node.js日志：`pm2 logs fxhook-diary`
3. 检查端口3000是否被占用：`netstat -tuln | grep 3000`

### Q4: CORS跨域错误

**解决方案**：
1. 检查 `.env` 中的 `ALLOWED_ORIGINS` 是否包含你的前端域名
2. 确保前端域名和实际访问域名完全一致（包括http/https）
3. 重启Node.js服务：`pm2 restart fxhook-diary`

### Q5: API请求返回404

**解决方案**：
1. 检查 `diaryAPI.js` 中的 `API_BASE_URL` 是否正确
2. 确保反向代理配置正确
3. 测试直接访问：`curl http://localhost:3000/api/health`

---

## 十二、性能优化

### 1. 开启Nginx缓存

在网站设置 → **配置文件** 中添加：

```nginx
# 在server块中添加
location ~* \.(jpg|jpeg|png|gif|ico|css|js)$ {
    expires 30d;
    access_log off;
}
```

### 2. 开启Gzip压缩

Nginx默认已开启，可在配置文件中确认：

```nginx
gzip on;
gzip_types text/plain text/css application/json application/javascript text/xml;
```

### 3. PM2集群模式（可选）

如果服务器CPU核心数较多：

```bash
pm2 delete fxhook-diary
pm2 start server.js -i max --name fxhook-diary
pm2 save
```

---

## 十三、维护清单

### 每天
- [ ] 查看PM2日志，确认无异常错误
- [ ] 检查MongoDB服务状态

### 每周
- [ ] 查看服务器磁盘空间
- [ ] 检查备份文件是否正常生成

### 每月
- [ ] 更新系统包：`apt update && apt upgrade`
- [ ] 检查宝塔面板更新
- [ ] 审查访问日志

---

## 完成！🎉

你的日记系统已成功部署在宝塔面板上！

**访问地址**：
- 前端：`https://fxhook.cn/pages/diary.html`
- 管理后台：`https://fxhook.cn/pages/diary-admin.html`
- API：`https://api.fxhook.cn/api`

**默认管理员密码**：在 `.env` 文件中设置的 `ADMIN_PASSWORD`

---

有问题欢迎查看宝塔面板的错误日志或PM2日志进行排查！
